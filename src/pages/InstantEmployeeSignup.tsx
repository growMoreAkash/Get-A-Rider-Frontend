...
const InstantEmployeeSignup = () => {
    ...
    const handleVerifyNumber = async () => {
        ...
        const employee = employees.find((employee: any) => employee.phone === fullPhoneNumber);

        //////.log(employee, 'employee ka test');

        if (employee) {
            // If phone is found and phoneVerified is true, skip OTP verification and proceed to password setup
            if (employee.phoneVerified) {
                setEmployeeId(employee._id); // Set employee ID for the signup
                ...
                try {
                    await axios.post(`${host}/signupEmployee`, { phone: fullPhoneNumber }, {
                        headers: {
                            Authorization: `Bearer ${token}`,
                        },
                    });
    ...
    const handleOtpSubmit = async () => {
        ...
        try {
            const response = await axios.post(`${host}/verifyOtpEmployee`, {
    ...
    const handlePasswordSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await axios.post(`${host}/addPasswordEmployee`, {
                password: password,
                phone: '+91' + phoneNumber,
                entityId: employeeId,
            }, {
    ...
    return (
        <div className="flex justify-center flex-col">
            ...
            <div className="mt-6">
                <TypeBrandCategoryList columns={employeeColumns} actions={null} data={employees} onRefresh={onRefresh} onViewLog={onViewLog} onTrashed={onTrashed} />
            </div>
        </div>
    );
};

export default InstantEmployeeSignup;
